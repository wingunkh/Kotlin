...
fun variable(){ //<코틀린 변수>
    ...
    val data5:Int by lazy { //lazy 키워드로 초기값을 할당하지 않고 선언 가능하다. by lazy { } 형식으로 선언하며 소스에서 변수가 사용되는 순간 중괄호로 묶은 부분이 자동실행되며 그 결과값이 초기값으로 설정된다. 단, val 키워드로 선언한 변수만 가능하다.
        println("lazy 키워드를 사용하였습니다.")
        10
    }

    var data6:Int=10 //코틀린의 모든 변수는 객체다. 그러므로 사실 Int 키워드는 데이터타입이 아니라 객체타입 즉, 클래스다.
    var data7:Int?=null //그렇기 때문에 null을 대입할 수도 있고,
    var data8=data1.plus(10) //객체의 메서드를 호출할 수도 있다.
}

fun string(){ //<코틀린 문자열>
    var str1:String="Hello \n world" //큰따옴표로 문자열 표현시 줄 바꿈이나 들여쓰기 등을 표현하려면 이스케이프 시퀀스를 입력해야 한다.
    var str2:String="""
        Hello
        World
...
fun function(parameter:Int):Int{ //<코틀린 함수>
    //함수 리턴 타입은 생략시 자동으로 Unit 타입이 적용된다. Unit 타입은 함수에서 리턴 값이 없음을 명시적으로 나타내는 타입이다.
