...
class Ctor (name:String){ //주 생성자는 클래스 선언부에 선언하며 필수는 아니지만 한 클래스에 하나만 생성 가능하다. 또한 주 생성자를 선언할 때 constructor 키워드를 생략할 수 있다.
    init{
        println("나는야 주 생성자")
    } //init 키워드를 이용해 주 생성자의 본문을 구현할 수 있다. init 키워드로 지정한 영역은 객체 생성 시 자동 실행된다.
